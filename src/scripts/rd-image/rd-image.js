...
import * as tomplate from 'tomplate';
import * as imageInput from './ImageInput';

let BlockEmbed = Quill.import('blots/block/embed');

export class RdImageBlot extends BlockEmbed {

    static create(data) {
        let node = super.create();
        console.warn("create=", data);
        node._data = data;
        node._editForm = imageInput.createImageInput(node);
        node.appendChild(buildImgElement(node));
...
RdImageBlot.blotName = 'rd-image';
RdImageBlot.tagName = 'div';
RdImageBlot.className = 'rd-image';

function buildImgElement(node) {
    const imageElement = tomplate(
        '<div class="image-wrapper"><img src="' + node._data + '" onerror="{onError()}"/>' +
        ' <div class="block-control" contenteditable="false">' +
        '  <button class="block-control-remove rd-button" onclick="{onRemove()}">-</button>' +
        ' </div>' +
        '</div>',
        {
            onError: function () {
            ...
            onRemove: function () {
                quill.deleteText(quill.getIndex(Quill.find(node)), 1);
            }
        });
    return imageElement;
}
